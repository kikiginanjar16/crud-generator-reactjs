...
const RevenueMetricForm: React.FC<RevenueMetricFormProps> = ({ initialValues, onSubmit, onCancel }) => {
  const [form] = Form.useForm();

  const handleSubmit = async (values: any) => {
    try {
      if (initialValues && initialValues.id) {
        await RevenueMetricService.update(initialValues.id, values);
        message.success('RevenueMetric updated successfully');
      } else {
        await RevenueMetricService.create(values);
        message.success('RevenueMetric created successfully');
      }
      onSubmit(values);
      form.resetFields();
    } catch (error) {
      message.error('Failed to save revenuemetric');
    }
  };

  return (
    <div className="revenuemetric-form-container">
      <Form
        form={form}
        layout="vertical"
